...
uint8_t buffercnt = 0;
uint8_t cmdID = 0;
uint8_t cmddata[10] = {0,17,10,15,29,11,11,12,25,0};
...
{  	
	if(receiving) 
	{
		...
		{
			if (myVerify_CRC16_Check_Sum(buffer, cmddata[cmdID])) 
			{
				Judge_Refresh(cmdID);
			}
			receiving = 0;
...
void Judge_Refresh(uint8_t cmd)
{
	...
	{
		b = (unsigned char*)&extPowerHeat.chassisVolt;
		c[0] = buffer[7];c[1] = buffer[8];c[2] = buffer[9];c[3] = buffer[10];
		for(int i = 0; i<4; i++){
			b[i] = (unsigned char)c[i];
		}
		
		b = (unsigned char*)&extPowerHeat.chassisCurrent;
		c[0] = buffer[11];c[1] = buffer[12];c[2] = buffer[13];c[3] = buffer[14];
		for(int i = 0; i<4; i++){
			b[i] = (unsigned char)c[i];
		}
		
		b = (unsigned char*)&extPowerHeat.chassisPower;
		c[0] = buffer[15];c[1] = buffer[16];c[2] = buffer[17];c[3] = buffer[18];
		for(int i = 0; i<4; i++){
			b[i] = (unsigned char)c[i];
		}
		
		b = (unsigned char*)&extPowerHeat.chassisPowerBuffer;
		c[0] = buffer[19];c[1] = buffer[20];c[2] = buffer[21];c[3] = buffer[22];
		for(int i = 0; i<4; i++){
			b[i] = (unsigned char)c[i];
		}
